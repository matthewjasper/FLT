...
variable {A : Type*} [CommSemiring A] (S : Submonoid A)
  ...
  (L : Type*)[CommSemiring L] [Algebra B L] [Algebra A L] [Algebra K L] [IsScalarTower A B L]
    [IsScalarTower A K L]
    [IsLocalizedModule (M:=B) (M':=L) S (Algebra.linearMap B L)]
...
theorem tensorProduct_ext {g h : L ⊗[K] M →ₗ[K] P}
  ...
  have heq := H x m
  rwa [← hl, ← IsScalarTower.algebraMap_smul K, ← TensorProduct.smul_tmul', map_smul,
    map_smul, IsUnit.smul_left_cancel <| map_units K s] at heq
